...
window.ShearPhoto.MINGGE(function() {
	//██████████重要设置████████████████
	var relativeUrl = contextPath+"/resources/com/ballchen/education/sharephoto"; //你不要在后面加斜杠，系统会自动给你加上斜杠，不信看下面！   index.html的JS引用路径自己改，很简单的说
	/*
	...
	Shear.config({
		...
		url: contextPath+ "/sharePhotoController/shearphoto", //后端处理地址，保证正确哦，这是常识，连这个地址都能写错，你就是菜B，已经在本版本中帮你加入相对路径，你基本不用改这里了
	...
	var up = new ShearPhoto.frameUpImg({
		...
		url: contextPath+ "/sharePhotoController/shearphoto", //HTML5切图时，不会用到该文件，后端处理地址，保证正确哦，这是常识，连这个地址都能写错，你就是菜B，已经在本版本中帮你加入相对路径，你基本不用改这里了
	...
	try {
		webcam.init_config = {
			...
			webcam.set_api_url(contextPath+ "/sharePhotoController/uploadCameraPhoto"); //拍照连接后端的URL文件，后端处理地址，保证正确哦，这是常识，连这个地址都能写错，你就是菜B，已经在本版本中帮你加入相对路径，你基本不用改这里了
