...
CREATE TABLE `ballchen_t_category`(
  ...
  KEY IDX_USER_BASIC_ID(`USER_BASIC_ID`)
)ENGINE =InnoDB AUTO_INCREMENT=2 CHARSET =utf8 COMMENT ='课程分类表';
...
CREATE TABLE `ballchen_t_course_chapter` (
  ...
  `PLAN_BEGIN_TIME` TIMESTAMP NOT NULL COMMENT '计划开始时间',
  `PLAN_END_TIME` TIMESTAMP NOT NULL COMMENT '计划结束时间',
  `REAL_BEGIN_TIME` TIMESTAMP COMMENT '实际开始时间',
  `REAL_END_TIME` TIMESTAMP COMMENT '实际结束时间',
  `COURSE_ID` VARCHAR(50) NOT NULL COMMENT '课程ID',
  PRIMARY KEY (`ID`),
  KEY IDX_CREATE_TIME(`CREATE_TIME`),
