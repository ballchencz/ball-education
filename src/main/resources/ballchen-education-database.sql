...
CREATE TABLE `ballchen_t_course`(
  ...
  `COURSE_INTRO` VARCHAR(500) COMMENT '课程简介',
  `COURSE_INFO` VARCHAR(500) COMMENT '课程须知',
...
/*创建知识点表*/
DROP TABLE IF EXISTS `ballchen_t_knowledge_point`;
CREATE TABLE `ballchen_t_knowledge_point`(
  `ID` VARCHAR(50) NOT NULL COMMENT '主键ID',
  `CREATE_TIME` TIMESTAMP NOT NULL DEFAULT current_timestamp COMMENT '创建时间',
  ...
  CONSTRAINT `FK_T_KNOWLEDGE_POINT_T_COURSE_ID` FOREIGN KEY (`COURSE_ID`) REFERENCES `ballchen_t_course` (`ID`)
)ENGINE =InnoDB AUTO_INCREMENT=2 CHARSET =utf8 COMMENT '知识点表';
...
CREATE TABLE `ballchen_t_course_chapter` (
  ...
  `PLAN_BEGIN_TIME` DATETIME COMMENT '计划开始时间',
...
/*创建上课地址表*/
DROP TABLE IF EXISTS `ballchen_t_address`;
CREATE TABLE  `ballchen_t_address`(
  `ID`  VARCHAR(50) NOT NULL COMMENT '主键ID',
  `CREATE_TIME` TIMESTAMP   NOT NULL DEFAULT current_timestamp COMMENT '创建时间',
  ...
  `MARK` VARCHAR(500) COMMENT '备注',
  `COURSE_CHAPTER_ID` VARCHAR(50) COMMENT '课程章节ID',
  PRIMARY KEY (`ID`),
  KEY IDX_ADDRESS_CREATE_TIME(`CREATE_TIME`),
  KEY IDX_ADDRESS_COUNTRY(`COUNTRY`),
  KEY IDX_ADDRESS_CITY(`CITY`),
  KEY IDX_ADDRESS_DETAIL_ADDRESS(`DETAIL_ADDRESS`),
  KEY IDX_ADDRESS_POSTCODE(`POSTCODE`),
  CONSTRAINT `FK_T_ADDRESS_T_COURSE_CHAPTER_ID` FOREIGN KEY (`COURSE_CHAPTER_ID`) REFERENCES `ballchen_t_course_chapter`(`ID`)
)ENGINE =InnoDB AUTO_INCREMENT=2 CHARSET =utf8 COMMENT '上课地址表';
