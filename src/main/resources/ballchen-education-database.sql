...
CREATE TABLE `ballchen_t_authorization`(
  `ID` VARCHAR(50) NOT NULL COMMENT '主键ID',
  `CREATE_TIME` TIMESTAMP NOT NULL DEFAULT current_timestamp COMMENT '创建时间',
  `AUTHORIZATION_NAME` VARCHAR(200) UNIQUE NOT NULL COMMENT '权限名称',
  `AUTHORIZATION_CODE` VARCHAR(200) UNIQUE NOT NULL  COMMENT '权限代码',
  `WORK_URL` VARCHAR(200) NOT NULL COMMENT '作用地址',
  `MARK` VARCHAR(500),
  PRIMARY KEY (`ID`),
  KEY IDX_CREATE_TIME(`CREATE_TIME`),
  KEY IDX_AUTHORIZATION_NAME(`AUTHORIZATION_NAME`),
  KEY IDX_AUTHORIZATION_CODE(`AUTHORIZATION_CODE`),
  KEY IDX_WORK_URL(`WORK_URL`)
